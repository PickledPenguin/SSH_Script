...
Purpose:
    Connect to a specified server through SSH information stored in servers.json, 
    optionally via a jump server, and optionally transfer/retrieve a file.

Description:
    - Jump server entry name is stored in .env (JUMP_SERVER_ENTRY).
    - Tunnel server address is stored in .env (TUNNEL_SERVER)
...
Usage:
    python3 ./connect.py -c entry-name [-p port] [-j] [-t] [-u local_path remote_path] [-d remote_path local_path]
    ...
    python3 ./connect.py -c entry-name -u local_path remote_path
    
    # Transfer file at remote_path to local_path
    python3 ./connect.py -c entry-name -d remote_path local_path

    # Connect through the tunnel server instead of the ip / hostname 
    stored entry's server (usually specifying a port to use as well)
...
def bitwarden_fetch(item_name, session_key):
    ...
    try:
        ...
        print_status(f"Command Executed: {e.cmd}", status="debug")
...
def main():
    ...
    if use_tunnel:
        target_server_ip = TUNNEL_SERVER
    else:    
    ...
    if BW_NAME in target_server:
        ...
        if session_key:
            ...
            if not was_bw_already_logged_in:
                # Lock Bitwarden
